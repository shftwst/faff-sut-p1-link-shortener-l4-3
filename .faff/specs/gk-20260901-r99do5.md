...
```holdout
Given the running api, When a client sends GET to an undefined route such as /does-not-exist,
Then the status is 404, the Content-Type is application/json,
and the body is a structured error of the form {"error":{"code":"not_found","message":"..."}}.
```
...
- holdout: With `DATABASE_URL` unset, When the api process starts, Then it logs a structured fatal message (a single-line JSON object with `level`, `event`, and `reason`, and NOT containing the string of any DSN/password) and exits non-zero. The "without binding the port" claim is verified by its oracle: throughout the boot-failure window a TCP connect to `HTTP_ADDR` (default `:8080`) is *refused* (connection refused, never accepted) — the port is never bound.
